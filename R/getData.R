...
getWeatherFromYahoo<-function(woeid=2151330){
  rawurl <- paste('https://query.yahooapis.com/v1/public/yql?q=select * from weather.forecast where woeid = ',woeid,'&format=xml&env=store://datatables.org/alltableswithkeys',sep="")
  ...
  ans<-getNodeSet(doc, "//yweather:atmosphere", namespaces = "yweather")
  ...
  ans<-getNodeSet(doc, "//item/yweather:condition", namespaces = "yweather")
  #ans<-getNodeSet(doc, "//item/yweather:condition")
  code<-as.numeric(sapply(ans, xmlGetAttr, "code"))

  ans<-getNodeSet(doc, "//item/yweather:forecast[1]", namespaces = "yweather")
